...
Run 2_empirical_tree_processor.sh to edit species trees inferred in IQtree and generate parameters for downstream simulations.
...
Navigate to the 2_simulation_scripts directory and run 1_simulation_prep.sh to organize data and generate directories for downstream simulations. You may need to grant yourself access to the SimPhy module. You can do this by using the command "chmod +x simphy_XXXXX", X meaning the version you are using. Run 2_prep_simphy.sh to generate a list of SimPhy commands for simulating loci off of the species trees generated previously. Run those commands with 3_run_simphy.sh
